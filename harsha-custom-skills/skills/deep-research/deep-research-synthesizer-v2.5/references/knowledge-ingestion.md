...
5. Batch Processing
6. Error Handling
7. Ingesting Pre-Researched Data (Visualization Entry)
...
## Ingesting Pre-Researched Data (Visualization Entry)

When using the **Visualization Entry Point** (see SKILL.md → Entry Points), the skill receives pre-researched data instead of running Phases 1-3. This section defines how to parse, normalize, and prepare that data for Phase 3.5 (Knowledge Graph) and Phase 6 (Assembly).

### Supported Input Formats

#### Format 1: JSON Claims Array

The most structured format. Each claim maps directly to a knowledge entry.

```json
[
  {
    "title": "Product-Led Growth Reduces CAC",
    "content": "PLG companies report 30-50% lower customer acquisition costs compared to sales-led models because the product itself drives conversion...",
    "confidence": "HIGH",
    "source": "https://openviewpartners.com/plg-benchmarks-2025",
    "category": "Growth Strategy",
    "tags": ["PLG", "CAC", "growth", "SaaS"],
    "related": ["freemium-models", "activation-metrics"]
  }
]
```

**Parsing rules**:
- Map directly to internal knowledge entry format
- Validate required fields: `title`, `content` (minimum)
- Auto-generate `id` from title hash if missing
- Auto-generate `summary` from first sentence of content if missing
- Inherit `confidence` if provided; default to `MEDIUM` (0.7) if not
- Inherit `category` if provided; attempt auto-categorization from content if not
- Inherit `tags` if provided; auto-extract from content if not

#### Format 2: Markdown with Inline Citations

Common output format from McKinsey Research skill and other analysis tools.

```markdown
## Product-Led Growth

Product-led growth (PLG) is a business methodology where user acquisition, expansion,
and retention are driven primarily by the product itself [OpenView Partners](https://openviewpartners.com).

### Key Metrics
- Time-to-value (TTV): Median 4.2 minutes for top PLG companies [Source](url)
- Free-to-paid conversion: 2-5% is typical [Lenny's Newsletter](url)

### Challenges
Balancing self-serve with enterprise sales is the primary challenge...
```

**Parsing rules**:
- `## Headings` → entry titles
- Content under each heading → entry content
- `[Source](URL)` citations → entry source field (first citation becomes primary source)
- `### Sub-headings` → subcategory or merged into parent entry content
- Bullet lists → preserved in content
- If no explicit confidence markers: assign `MEDIUM` to all entries
- Look for confidence language: "well-established" → HIGH, "emerging evidence" → MEDIUM, "anecdotal" → LOW

#### Format 3: Structured Report Sections

Output from consultant-style analysis (McKinsey v3.0 output).

```
FINDING 1: Market Size Growing Rapidly
Confidence: HIGH
Sources: Gartner Report 2025, McKinsey Global Survey
Evidence: The global PropTech market reached $32B in 2025,
growing at 15.2% CAGR...

FINDING 2: Regulatory Headwinds in India
Confidence: MEDIUM
Sources: RERA Annual Report 2025
Evidence: New RERA amendments require...
```

**Parsing rules**:
- `FINDING N:` or numbered sections → entry titles
- `Confidence:` line → confidence field
- `Sources:` line → source field (comma-separated → array)
- `Evidence:` or remaining text → content field
- Auto-generate tags from title + content keywords

### Mapping External Claims to Internal Format

Regardless of input format, all claims must be normalized to the internal knowledge entry schema:

```python
def normalize_pre_researched_claim(raw_claim, index):
    """Convert any parsed claim to internal knowledge entry format."""
    entry = {
        'id': raw_claim.get('id') or generate_id(raw_claim.get('title', f'claim-{index}')),
        'title': raw_claim.get('title', f'Untitled Claim {index + 1}'),
        'content': raw_claim.get('content', raw_claim.get('evidence', '')),
        'summary': raw_claim.get('summary', extract_first_sentence(raw_claim.get('content', ''))),
        'category': raw_claim.get('category', 'Uncategorized'),
        'subcategory': raw_claim.get('subcategory', ''),
        'confidence': normalize_confidence(raw_claim.get('confidence', 'MEDIUM')),
        'source': raw_claim.get('source', raw_claim.get('sources', 'Pre-researched (no source provided)')),
        'tags': raw_claim.get('tags', extract_tags(raw_claim.get('content', ''))),
        'related': raw_claim.get('related', []),
        'gaps': raw_claim.get('gaps', []),
        'authority_tier': raw_claim.get('authority_tier', estimate_authority(raw_claim)),
        'origin': 'visualization-entry',  # Mark as pre-researched
        'original_source_skill': raw_claim.get('source_skill', 'unknown')
    }
    return entry
...
def normalize_confidence(value):
    ...
    valid = ['VERIFIED', 'HIGH', 'MEDIUM', 'LOW', 'UNKNOWN']
    return value if value in valid else 'MEDIUM'

def estimate_authority(claim):
    """Estimate authority tier from source information."""
    source = str(claim.get('source', '')).lower()
    if any(s in source for s in ['.gov', '.edu', 'nature.com', 'science.org', 'lancet']):
        return 'Tier 1'
    if any(s in source for s in ['mckinsey', 'gartner', 'forrester', 'reuters', 'bloomberg']):
        return 'Tier 2'
    if any(s in source for s in ['.com', '.org', 'medium.com', 'substack']):
        return 'Tier 3'
    return 'Tier 4'
```

### Confidence Score Assignment for Pre-Researched Data

| Scenario | Assigned Confidence | Rationale |
|----------|-------------------|-----------|
| Claim includes explicit confidence level | Inherit as-is | Trust the upstream skill's assessment |
| Claim has 3+ cited sources | HIGH | Multiple corroboration = high confidence |
| Claim has 1-2 cited sources | MEDIUM | Some evidence but not fully corroborated |
| Claim has no sources | LOW | No verifiable evidence provided |
| Numeric confidence (0.0-1.0) provided | Map to tier | ≥0.8 → HIGH, ≥0.5 → MEDIUM, ≥0.3 → LOW, <0.3 → UNKNOWN |

### Relationship Edge Detection from Pre-Researched Data

When ingesting pre-researched data, relationships must be inferred since the upstream skill may not have produced a knowledge graph:

1. **Explicit relationships**: If input includes `related` field, create edges directly
2. **Cross-references**: Scan content for mentions of other entry titles → create semantic edges
3. **Tag overlap**: Apply standard tag overlap algorithm (2+ shared tags → semantic edge)
4. **Category siblings**: Entries in same category → base semantic edge (strength 0.5)
5. **Dependency detection**: Scan for language patterns indicating prerequisites:
   - "requires understanding of X" → dependency edge to X
   - "builds on X" → dependency edge to X
   - "prerequisite: X" → dependency edge to X
   - "before learning X, understand Y" → dependency edge from X to Y

### Visualization Entry Processing Pipeline

```
INPUT (JSON / Markdown / Structured Report)
  │
  ├─ 1. FORMAT DETECTION
  │   └─ Identify input format (JSON array? Markdown? Structured report?)
  │
  ├─ 2. PARSING
  │   └─ Extract claims using format-specific parser
  │
  ├─ 3. NORMALIZATION
  │   └─ Map each claim to internal knowledge entry schema
  │   └─ Auto-generate missing fields (id, summary, tags)
  │   └─ Assign confidence scores (inherit or default)
  │
  ├─ 4. RELATIONSHIP INFERENCE
  │   └─ Detect edges from explicit references, tag overlap, content similarity
  │   └─ Detect dependency edges from language patterns
  │
  ├─ 5. VALIDATION (Light QA)
  │   └─ Check required fields present
  │   └─ Verify no duplicate entries
  │   └─ Flag entries with no source as LOW confidence
  │
  └─ 6. OUTPUT → Phase 3.5 (Knowledge Graph Building)
      └─ Entries array + inferred edges → standard graph pipeline
```
